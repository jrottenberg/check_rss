...
'''
check_rss v .3 - A simple Nagios plugin to check an RSS feed. Created to monitor status of cloud services.
...
def main(argv=None):
	...
	if (args.verbosity == '0' ):
		output = ': Posted {hourssinceposted} hrs ago ; {title}'.format(hourssinceposted=hourssinceposted, title=title) 
	elif (args.verbosity == '1' ):
		output = ': Posted {hourssinceposted} hrs ago ; Title: {title} ; Link: {link}'.format(hourssinceposted=hourssinceposted, title=title, link=link)
